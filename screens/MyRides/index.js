...
export default function MyRides({ navigation }) {
  ...
  useEffect(() => {
    AsyncStorage.getItem('loginData').then(data => {
      ...
      axios(`https://pigeon-dev2.herokuapp.com/driver/driver-orders-myRides/${data['driverEmail']}/${statusData}?dateDataurl=${date1}`)
        .then((response) => {
          console.log(
            `hcjhsac-=-=--=-=-=-=-=-=-=-=-=-=-=-=-=>>>>>> upcoming ${date1} `,
            response.data.data
          );
